...
def plot_temperature(in_temp, out_temp=None, ax=None, **kwads):
    ...
    lns1 = ax.plot_date(
        in_temp.index.to_pydatetime(), in_temp.values, "k.", label="in_temp"
    )
    ...
    if out_temp is not None:
        ax2 = ax.twinx()
        lns2 = ax2.plot_date(
            out_temp.index.to_pydatetime(), out_temp.values, "kx", label="out_temp"
        )
...
def plot_tilts(tilt_y, tilt_x, ax=None, **kwargs):
    ...
    ax.plot_date(tilt_x.index.to_pydatetime(), tilt_x.values, "k.", label="tilt_x")
    ax.plot_date(tilt_y.index.to_pydatetime(), tilt_y.values, "kx", label="tilt_y")
    ...
    ax.yaxis.set_major_locator(ticker.FixedLocator(range(-25, 25, 5), nbins=5))
...
def plot_setup(data, ax=None, **kwargs):
    ...
    for i, group in data.groupby("setup"):
        ...
        ax.text(
            ...
            yi,
            int(i),
            bbox=dict(facecolor="white", edgecolor="black", linestyle="solid"),
...
def plot_truncate(series, axes):
    for ax in axes:
        for _i, group in series.groupby(series):
...
def plot_drift(drift, ax=None):
    ...
    title = "Resid. stats.: "
    for n, x in zip(stats.index, stats.values, strict=False):
        title += rf"{n} = {x * 1000:.1f} $\mu$Gal   "
...
def plot_loop_processing(loop1, loop2):
    ...
    fig.gca().text(
        1.0,
        1.005,
        "Gravimeter: " + meter_name + " # " + str(loop1.data.meter_sn.unique()[0]),
    ...
    plot_temperature(data1.in_temp, out_temp, ax=fig.gca())
    ...
    for _i, group in data2.groupby("setup"):
    ...
    for ax in fig.axes:
        ax.xaxis.set_major_formatter(dates.DateFormatter("%H:%M"))
        ax.set_xlim(
            [dates.date2num(data1.index.min()), dates.date2num(data1.index.max())]
        )
