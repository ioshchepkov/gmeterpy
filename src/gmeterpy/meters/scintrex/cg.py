...
def _parse_header(line, header_fields):
    for key, value in header_fields.items():
        pattern = re.compile(rf"({value}):\s+(?P<{key}>.*)", re.IGNORECASE)
...
class _ScintrexCGBase(RelativeReadings):
    """Base class for Scintrex CG-5 and CG-6 data file."""
    ...
    def __init__(self, filename, use_drift=True, use_tide=True, **kwargs):
        ...
        df["c_drift"] = np.around(-delta_t.values * df.drift.values, decimals=4)
...
class ScintrexCG5(_ScintrexCGBase):
    """Read Scintrex CG-5 data file."""

    instrument_name = "ScintrexCG-5"

    _default_corrections: ClassVar[dict] = {"c_tide": ("c_tide", {})}
