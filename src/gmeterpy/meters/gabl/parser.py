...
def read_binary_drop(fname, dist_ivals=False, full=False, timezone=0):
    ...
    xtra, high, mid, low = np.asarray(data.reshape(int(len(data) / 4), 4).T, dtype=int)

    opt = {
        "time": datetime(1899, 12, 30)
        + timedelta(days=opt[0])
    ...
    # norming
    xtra_norm = np.asarray(
        255 / (xtra.max() - xtra.min()) * (xtra - xtra.min()), dtype=int
    )
    ...
    if dist_ivals or full:
        ...
        distance_intervals = opt["divider"] * index * opt["wavelength"] / 2
...
def load_from_path(path, add_to_meta=None, timezone=0):
    ...
    drops = []
    for _snc_n, snc_path in enumerate(snc_paths):
        ...
        for root, _, files in os.walk(snc_path):
            for f in files:
                if fnmatch.fnmatch(f, "SingleDrop*"):
                    nn = int(f.removeprefix("SingleDrop"))
                    srs_n = int(os.path.basename(root).removeprefix("Series"))
                    ...
                    time, distance, drop_meta = read_binary_drop(
                        os.path.join(root, f), dist_ivals=True, timezone=timezone
                    )
