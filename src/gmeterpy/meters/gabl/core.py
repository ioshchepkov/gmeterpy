...
import numpy as np
import pandas as pd
from scipy.stats import sigmaclip, t, ttest_ind_from_stats
from tqdm import tqdm

import gmeterpy.units as u
from gmeterpy.core.readings import Readings
from gmeterpy.corrections import (
...
_corrections = {
    "c_polar": (
        polar_motion_correction,
        {"xp": "xp", "yp": "yp", "lat": "lat", "lon": "lon"},
    ),
    "c_vac": (lambda x, y: x * y, {"x": "aero_coeff", "y": "vac"}),
    "c_atm": (
        atmospheric_pressure_correction,
        {"height": "height", "pressure": "pres", "barometric_factor": "baro_factor"},
    ),
    "c_dop": (
        finite_speed_of_light_correction,
        {"gravity": "g", "initial_velocity": "v0", "drop_duration": "drop_duration"},
    ),
}
...
def t_statistics(x, stderr, n, alpha=0.05):
    ...
    t_exp, pval = ttest_ind_from_stats(
        x[0], std[0], n[0], x[1], std[1], n[1], equal_var=False
    )
...
class GABLProject(Readings):
    def __init__(self, drops=None, *args, **kwargs):
    ...
    def get_drops(self, azimuth=None, seance=None, series=None, accepted=True):
        ...
        if azimuth is not None:
            data = list(filter(lambda x: x._model._meta["azimuth"] == azimuth, data))
        if seance is not None:
            data = list(filter(lambda x: x._model._meta["seance"] == seance, data))
        if series is not None:
            data = list(filter(lambda x: x._model._meta["series"] == series, data))
        if accepted:
    ...
    def proc_drops(self, before=0, after=0, **kwargs):
        ...
        if (
            np.any(
                [
                    data.lat.unique().size,
                    data.lon.unique().size,
                    data.height.unique().size,
                ]
            )
            > 1
        ):
    ...
    def filter(self, by=None, sigma=3.0):
        ...
        for _, group in data.reset_index(drop=True).groupby(by):
            group = group.copy()
            _, lower, upper = sigmaclip(group.g_result.values, low=sigma, high=sigma)
    ...
    def mean(self, by=None, corrections=True):

        if by is None:
            by = ["seance", "azimuth", "series"]
        if corrections:
        ...
        series_rej = self._data.loc[~self._data.accepted, cols].groupby(by)
        series = self._data.loc[self._data.accepted, cols].groupby(by)

        srs_mean = series.mean(numeric_only=True)
        srs_mean["time"] = series.apply(
            lambda x: x.index.min() + (x.index - x.index.min()).to_series().mean()
        )
    ...
    def get_residuals(self, by=None):
        if by is None:
            by = ["azimuth", "seance", "series"]
        out = []
        for idx, _group in self._data.groupby(by):
    ...
    def plot_residuals(self, by=None):
        if by is None:
            by = ["azimuth", "seance", "series"]
        plots = []
    ...
    def report(self):
        ...
        with open("drops.txt", "w") as f:
            f.write(
                data[cols]
                .sort_values("time")
                .set_index(["azimuth", "seance", "series", "nn"])
                .to_string(formatters=_formatters, sparsify=False)
            )
        ...
        if self._set._seances is not None:
            ...
            with open("seances.txt", "w") as f:
                f.write(
                    self._set._seances[cols]
                    .sort_values("time")
                    .to_string(formatters=_formatters)
                )
        ...
        with open("azimuths.txt", "w") as f:
            ...
            if len(self._set._directions) > 1:
                t_exp, pval, t_crit, _dof, passed = self._set._t_test
                f.write("Welch's t-test\n")
                f.write(f"t = {t_exp:.3f}, p-value = {pval:.3f}\n")
                f.write(f"t_critical = {t_crit:.3f}, alpha = {0.05}\n")
                if not passed:
                ...
                f.write("\n\n")
            f.write(self._set._final[cols[1:]].to_string(formatters=_formatters))
...
class GABLSet:
    ...
    def proc(self):
        ...
        if "seance" in self._data.index.names:
            seances = self._data.reset_index(drop=False).groupby(["azimuth", "seance"])
        ...
        dir_result = dir_grouped.apply(lambda x: group_wmean(x, weighted=False))
        ...
        fin_result = pd.DataFrame(
            group_wmean(dir_result, weighted=False).to_dict(), index=[0]
        )
        ...
        if len(self._directions) > 1:
            # t-statistics
            self._t_test = t_statistics(
                dir_result.g_result.values,
                dir_result.err.values,
                dir_result.accepted.values,
            )
