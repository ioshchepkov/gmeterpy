...
from gmeterpy.core.dmatrices import dmatrix_dummy, dmvstack
from gmeterpy.core.gvalues import GravityValues
from gmeterpy.core.relative import RelativeReadings, Ties
...
class Network:
    ...
    def add_data(self, data, **kwargs):
        ...
        if isinstance(data, (RelativeReadings, Ties)):
            ...
            if fix_rgmeters != "all":
                ...
                det_rgmeters = [x for x in self.rgmeters if x not in self.fix_rgmeters]
            ...
            if fix_agmeters != "all":
                ...
                if len(fix_agmeters) == 0:
                    ...
                    if w_offset:
                        weights = (
                            self.fix_stations.data[["meter_sn", "weight"]]
                            .groupby("meter_sn")
                            .mean()
                            .sort_index()
                        )
                    ...
                    self.dm, self.wm, self.l = dmvstack(
                        self.dm,
                        self.wm,
                        self.l,
                        offset,
    ...
    def det_stations(self):
        out = np.array(
            [x for x in self.stations if x not in self.fix_stations.stations()]
        )
        return out
    ...
    def adjust(self, method="WLS", **kwargs):
        ...
        return AdjustmentResults(self, model.fit())


class NetworkAdjustmentResults(AdjustmentResults):
    ...
    def ties(self):
        ...
        data["En"] = (data.resid.abs() / np.sqrt(data.stdev**2 + data.u_adj**2)).round(
            1
        )
    ...
    def fixed_stations(self):
        ...
        fix_data["En"] = (
            fix_data.resid.abs() / np.sqrt(fix_data.stdev**2 + fix_data.u_adj**2)
        ).round(1)
    ...
    def gravity(self, stations="all"):
        ...
        df = pd.DataFrame({"g": self.params.loc[idx], "stdev": self.sigma.loc[idx]})
        pd.options.display.float_format = "{:.4f}".format
        return df
    ...
    def adjusted_ties(self):
        ...
        for i, s1 in enumerate(st):
            for s2 in st[i + 1 :]:
                ...
                adj_tie = pd.Series(
                    {"from": s1, "to": s2, "delta_g": delta_g, "stdev": u}
                )
                adj_ties = adj_ties.append(adj_tie, ignore_index=True, sort=True)
        return adj_ties
    ...
    def report(self, scale=1.0):
        ...
        with open("adjreport.txt", "w") as f:
            ...
            if self.model._method in ("OLS", "WLS"):
                ...
                f.write(f"A priori st.dev. u. weight: {scale}\n")
                f.write(f"Ref. var.: {self.s0**2}\n")
                f.write(f"Ref.st. dev.: {self.s0}\n")
                f.write(f"X^2 = {chisq}\n")
            ...
            if self.model.fix_stations:
                ...
                f.write(f"Total: rms = {rms_resid:.4f}, n = {n_resid:.0f}\n")
            ...
            if hasattr(self.model, "ties"):
                ...
                f.write(
                    f"\nTotal: rms = {rms(self.ties().resid):.4f}, n = {len(self.ties()):.0f}\n"
                )
                f.write("\n\nResiduals statistics by ties\n")
                f.write(28 * "=" + "\n")
                f.write(
                    self.resid_stats(["from", "to"])
                ...
                df = pd.DataFrame()
                for _i, group in grouped:
                    if len(group) > 1:
                        group = group.copy()
                        group["from_mean"] = np.round(
                            (group.delta_g - group.delta_g.mean()), 4
                        )
            ...
            with open("ties_adjusted.txt", "w") as f:
                pd.options.display.float_format = "{:.4f}".format
                f.write(
                    self.adjusted_ties()[["from", "to", "delta_g", "stdev"]].to_string(
                        index=False
                    )
                )
