...
def dmatrix_gravity(data, by=None, column="name", return_type="dataframe"):

    if by is not None:
        dm_data = by + "_" + data[by].astype(str) + "_" + data[column].astype(str)
    else:
        dm_data = data[column]

    dm = dmatrix_dummy(dm_data, return_type="dataframe")
...
def dmatrix_calibration(
    ...
    if by is not None:
        dm_data = by + "_" + data[by].astype(str) + "_" + data[meter_column].astype(str)
    else:
    ...
    dm = dm * np.tile(
        data[calibration_column].values.reshape((dm.shape[0], 1)), dm.shape[1]
    )
...
def dmatrix_drift(
    ...
    if by is not None:
        ...
        for n, group in data.groupby(by=by):
            ...
            colnames = [str(by) + "_" + str(n) + "_" + name for name in dmd.columns]

            dmd.columns = colnames
            dm.append(dmd)
        dm = concat(dm, axis=1).fillna(0)
    else:
        dm = dmatrix_drift_model(
            data,
            time_column=time_column,
            reference_time=reference_time,
            return_type="dataframe",
...
def dmatrix_relative_gravity_readings(
    readings,
    gravity=True,
    gravity_column="name",
    gravity_by=None,
    ...
    if gravity:
        dm.append(
            dmatrix_gravity(
                readings, by=gravity_by, column=gravity_column, return_type="dataframe"
            )
        )
...
def dmatrix_ties(ties, fix_meters="all"):
    ...
    if fix_meters != "all":
        dmrg = dmatrix_calibration(
            ties.data.meter_sn.values, ties.data.delta_g.values, return_type="dataframe"
        )
