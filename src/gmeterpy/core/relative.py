...
from gmeterpy.core.dmatrices import dmatrix_relative_gravity_readings, dmatrix_ties
...
def _closures(df, root=None):
    ...
    network = nx.from_pandas_edgelist(
        df, "from", "to", edge_attr="delta_g", create_using=nx.DiGraph()
    )
    ...
    for closure in basis:
        closure_sum = 0
        for node1, node2 in zip(closure, closure[1:] + closure[:1], strict=False):
...
class RelativeReadings(Readings):
    ...
    def setup_id(self):
        ...
        idx = np.concatenate(
            (
                [0],
                np.where(self.data["sid"][:-1].values != self.data["sid"][1:].values)[0]
                + 1,
                [len(self.data)],
            )
        )

        rng = [(a, b) for a, b in zip(idx, idx[1:], strict=False)]
        setup = []
    ...
    def adjust(
        ...
        **kwargs,
        ...
        if sm_model_args is None:
            sm_model_args = {"M": sm.robust.norms.HuberT()}
        if drift_args is None:
            drift_args = {"drift_order": 1}
        t0 = self.data.jd.min()
        # readings._data['dt0'] = readings.data.jd - t0

        # design matrix
        dm, _, y = self.dmatrices(gravity=gravity, drift_args=drift_args, **kwargs)
...
class RelativeReadingsResults(AdjustmentResults):
    ...
    def drift(self):
        self.res.params[self.res.params.index.str.startswith("drift")]
    ...
    def ties(self, ref=None, sort=False):
        ...
        adjg = pd.DataFrame(
            {"g": self.res.params[stations], "stdev": self.res.bse[stations]}
        )
        if sort:
        ...
        if ref is None:
            ...
            if ref not in stations:
                raise Exception(f"Station {ref} does not exist.")
            else:
                from_st = ref
                to_st = adjg[adjg.index != ref].index.values
                delta_g = (adjg.loc[to_st].g - adjg.loc[from_st].g).values
        elif isinstance(ref, list):
            from_st, to_st = [p for p in zip(*ref, strict=False)]
            delta_g = [
                adjg.loc[p2].g - adjg.loc[p1].g
                for p1, p2 in zip(from_st, to_st, strict=False)
            ]
    ...
    def report(self):
        ...
        out += self.res.summary2().tables[0].to_string(index=False, header=False)
...
class Ties:
    ...
    def stations(self):
        return np.unique(np.append(self.data["from"].values, self.data["to"].values))
    ...
    def closures(self, by=None, root=None):
        ...
        if by is None:
            ...
            out = _closures(df.delta_g.mean().reset_index(drop=False), root=root)
        else:
            out = {}
            for i, group in self.data.groupby(by):
                df = group.groupby(["from", "to"])
                cl = _closures(df.delta_g.mean().reset_index(drop=False), root=root)
                if cl:
                    out[str(i)] = cl
