...
def polynomial_vgg_correction_uncertainty(h1, h2, cov_params):
    ...
    a = {
        coeff: h2 ** (n + 1) - h1 ** (n + 1)
        for n, coeff in enumerate(cov_params.columns)
    }
    u = np.sum(
        [
            a[c1] * a[c2] * cov_params[c1][c2]
            for c1, c2 in itertools.product(cov_params.columns, repeat=2)
        ],
        axis=0,
    )
    return np.sqrt(u)
...
def fit_floating_gravity(data, deg=2, **kwargs):
    ...
    df = pd.DataFrame(
        {
            "g": np.concatenate((np.zeros_like(data.delta_g), data.delta_g)),
    ...
    h0 = [f"h({hi / 1000:,.3f})" for hi in np.asarray(h_level_1)]
...
def generate_report(report_file, data, res, gp, station):
    ...
    h_control = np.array(
        [0.000, 0.05, 0.130, 0.270, 0.700, 0.720, 0.900, 1.000, 1.200, 1.300, 1.4]
    )
    with open(report_file, "w") as f:
        f.write(
            "Results of the polynomial fit of the vertical gravity gradients at "
            + station
            + "\n\n"
        )
        f.write("Data:\n")
        ...
        f.write("Fit summary:\n" + 12 * "*" + "\n")
        f.write(f"{res.nobs:2.0f} observations -\n")
        f.write(f"{len(res.params):2.0f} parameters =\n")
        f.write(f"{res.df_resid:2.0f} degrees of freedom\n\n")

        if hasattr(res, "ssr"):
            f.write(f"Sum of squared residuals: {res.ssr}\n\n")

        pd.options.display.float_format = "{:.2f}".format
        f.write(res.summary2(alpha=0.05).tables[1].to_string() + "\n\n")

        f.write("Covariance matrix\n")
        f.write(res.cov_params().to_string(float_format="{:.6f}".format) + "\n\n")

        f.write("Prediction table:\n")
        f.write(18 * "*" + "\n")
        f.write("{:17}\t{:^}\t\t{:>}\n".format("Heights", "Gravity diffs", "Gradients"))
        cov_params = res.cov_params().iloc[-gp.order :, -gp.order :]
        for hc in h_control:
            ha = h_control[np.where(h_control > hc)]
            for h1, h2 in zip(np.repeat(hc, len(ha)), ha, strict=False):
                dg = polynomial_vgg_correction(h1, h2, gp)
                se_dg = polynomial_vgg_correction_uncertainty(h1, h2, cov_params)
                grad = dg / (h2 - h1)
                se_grad = se_dg / abs(h2 - h1)
                f.write(
                    (
                        "{:.3f} --> {:.3f} m,\t{:6.1f} +/- {:.1f} uGal,"
                        + "\t{:6.1f} +/- {:.1f} uGal/m\n"
