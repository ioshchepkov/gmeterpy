...
def main():
    conf_parser = argparse.ArgumentParser(
        description="Process relative measurements",
    ...
    # Merge files
    parser.add_argument(
        "--stations", type=open, help="specify file with station inforamtion"
    )
    ...
    parser.add_argument(
        "--drift-order", type=int, help="polynomial order for a drift function"
    )

    # Corrections
    parser.add_argument(
        "--tide-model", type=str, help="tide model to calculate tide correction"
    )
    parser.add_argument("--tide", type=open, help="file with tides")
    parser.add_argument(
        "--calibration", type=open, help="file with calibration coefficients"
    )
    parser.add_argument("--pressure", type=open, help="file with pressure measurements")
    ...
    if args.calibration is not None:
        cparams = pd.read_csv(args.calibration, delim_whitespace=True).drop(
            "stdev", axis=1
        )
    ...
    if tide_model in ("tamura", "atlantida", "custom"):
        ...
        if tide_model == "tamura":
            ...
            for name, group in readings.data.groupby("name"):
                c_tide = (
                    interpolate(tides[tides.name == name]["tide"], group.index) / 1000
                )
    ...
    if args.pressure is not None:
        ...
        readings.data["c_atm"] = (
            atmospheric_pressure_correction(readings.data.height, readings.data["pres"])
            / 1000
        ).round(4)
