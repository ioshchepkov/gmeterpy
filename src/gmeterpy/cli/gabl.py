...
def main():
    ...
    parser.add_argument("--elev", type=float, help="height above sea level")
    parser.add_argument("--timezone", type=float, help="timedelta from UTC in hours")
    parser.add_argument(
        "--baro-factor", type=float, help="barometric admittance factor, default is 0.3"
    )

    # Instrument
    parser.add_argument("--meter-type", type=str, help="type of the gravimeter")
    parser.add_argument("--meter-sn", type=str, help="s/n of the gravimeter")

    # Corrections
    parser.add_argument("--tide", type=argparse.FileType("r"), required=True)

    # Processing
    parser.add_argument("--tdb", type=int, help="truncate drop in the beginning")
    parser.add_argument("--tda", type=int, help="truncate drop in the end")
    parser.add_argument("--sigma", type=float, help="rejection level, default is 3.0")
    parser.add_argument(
        "--reject-by",
        type=str,
    ...
    parser.add_argument(
        "--drop-model", type=str, help="regression model for drop fitting"
    )
