...
from gmeterpy.corrections.atmosphere import (
...
height, t_pressure, t_atm_corr = np.loadtxt(
    fname, usecols=(0, 1, 2), unpack=True, dtype=np.float32
)
...
def test_normal_pressure():
    ...
    np.testing.assert_array_almost_equal(pressure.value, t_pressure.value, decimal=1)

    # Numerical formula from IAGBN
    t_pn_num = 101325 * (1 - 0.0065 * height.value / 288.15) ** 5.2559

    np.testing.assert_array_almost_equal(pressure.value, t_pn_num, decimal=1)
...
def test_atmospheric_pressure_corrections():
    ...
    np.testing.assert_array_almost_equal(atm_corr.value, t_atm_corr.value, decimal=1)
