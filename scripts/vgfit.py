...
parser = argparse.ArgumentParser(
        description='Polynomial fit of the vertical gravity gradients')
parser.add_argument('-i', '--input', type=argparse.FileType('rt'),
...
gp = np.poly1d(
        np.concatenate([params.values[::-1], np.zeros(1)]),
        variable='h')
